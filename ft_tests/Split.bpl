  ...
  modifies C, R, M, I, H;
{
  ...
  assume C[out2] == 0;
  c := 0;
  assert {:msg "5.13: Initialization might not establish the invariant (#0)"} c == C[out1];
  assert {:msg "6.20: Initialization might not establish the invariant (#1)"} R[in] == C[out1];
  assert {:msg "7.20: Initialization might not establish the invariant (#2)"} C[out1] == C[out2];
  assert {:msg "9.21: Initialization might not establish the invariant (#3)"} (forall i: int :: 
    (0 <= i) && (i < C[out1]) ==> (H[M[out1][i]][sqn#] == i)
  );
  assert {:msg "10.21: Initialization might not establish the invariant (#4)"} (forall i: int :: 
